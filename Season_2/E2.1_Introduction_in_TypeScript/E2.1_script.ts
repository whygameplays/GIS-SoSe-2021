...
function randoooom2values(): void {
...
function faculty(v: number): number {
    if (v < 1) {
        return 1;
    } else {
        let output: number = 1;
        for (let i: number = 2; i <= v; i++) {
            output *= i;
        }
        return output;
    }
...
function chesscheckerboard(height: number, width: number): void {
    let output: String = "";
